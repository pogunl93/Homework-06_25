...
split_text = string_of_text.gsub(/[.!;:]/, "")
final_text = split_text.downcase.split (" ")

final_text.each do |words|
	if !hash_of_words.has_key?(words) == true
		hash_of_words[words] = 1
	else
		hash_of_words[words] +=1
	end
end
